 ...
 * @param {object} data
 */
 function updateMapMaterialFromData(
  materialName,
  dataName,
  shader,
  data
 ...
 */
 function updateEnvMap(shader, data) {
  ...
  if (sphericalEnvMap) {
    src = sphericalEnvMap;
    warn(
      "`sphericalEnvMap` property is deprecated, using spherical map as equirectangular map instead. " +
  ...
  envMapPromises[src] = new Promise(function (resolve) {
    srcLoader.validateEnvMapSrc(
      src,
      function loadCubeMap(srcs) {
        el.sceneEl.systems.material.loadCubeMapTexture(
          srcs,
          function (texture) {
            texture.mapping = refract
 ...
 */
 function handleTextureEvents(el, texture) {
  ...
  texture.addEventListener("dispose", function cleanupListeners() {
    texture.image.removeEventListener(
      "loadeddata",
 ...
 */
 function isCompatibleTexture(texture, source) {
  ...
  return (
    texture.isTexture && !texture.isCanvasTexture && !texture.isVideoTexture
  );
}
...
module.exports.updateDistortionMap = updateDistortionMap;
module.exports.updateMapMaterialFromData = updateMapMaterialFromData;
module.exports.updateEnvMap = updateEnvMap;
