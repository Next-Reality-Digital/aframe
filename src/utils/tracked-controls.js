 ...
 */
function checkControllerPresentAndSetup(
  component,
  idPrefix,
  queryObject
) {
...
function findMatchingControllerWebXR(
  controllers,
  idPrefix,
  handedness,
  index,
  ...
  for (i = 0; i < controllers.length; i++) {
    ...
    if (handTracking) {
      ...
      if (iterateProfiles) {
        ...
        controllerMatch =
          profiles.length > 0 && profiles[0].startsWith(idPrefix);
      }
    }
    if (!controllerMatch) {
      continue;
    ...
    controllerHasHandedness =
      controller.handedness === "right" || controller.handedness === "left";
...
module.exports.checkControllerPresentAndSetup = checkControllerPresentAndSetup;
...
module.exports.onButtonEvent = onButtonEvent;
module.exports.emitIfAxesChanged = emitIfAxesChanged;
